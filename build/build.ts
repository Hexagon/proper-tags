...
if (Deno.args[1] === "clean") {
  ...
  await build({
    entryPoints: [resolve(relativeProjectRoot, "src/proper-tags.ts")],
    bundle: true,
    minify: true,
    sourcemap: false,
  }, [
    ...
    },
    {
      entryPoints: [],
      stdin: {
        contents: 'import { Cron } from "./proper-tags.ts";module.exports = Cron;',
        resolveDir: "./src/",
      },
      outfile: resolve(resolvedDistPath, "proper-tags.umd.js"),
      platform: "browser",
      format: "iife",
      globalName: "Cron",
    },
  ...
  await cp(
    resolve(resolvedDistPath, "proper-tags.d.ts"),
    resolve(resolvedDistPath, "proper-tags.d.cts"),
  );
